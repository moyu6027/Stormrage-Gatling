...
class ComssDownloadObjectSimulation extends Simulation {
  ...
  /*** NORMAL SCENARIOS ***/
  def mScenario() = {
    exec(DownloadObject.downloadObjectStreamWithEncryption)
...
//  println(s"List of Files is : ${GatlingHelper.getListOfFiles("src/gatling/resources/data")}")
  ...
  /***************** PREPARATION ************************/
  /*** NORMAL SCENARIOS ***/
  val mScn = scenario("COMSS Download Object Scenario")
    .exec(mScenario())
